...
class CompletedSeed:
    def __init__(
        self, config, qbitclient, name, hash, content_path, save_path, completion_on
    ):
        ...
        self.save_path = os.path.normpath(
            save_path
        )  # path to category, if in a category
        self.time_complete = self.elapsed_time(completion_on)
        self.genre = os.path.basename(os.path.dirname(self.save_path))
    ...
    def delete_content(self):
        if self.delete_from_client:
            self.qbitclient.torrents_delete(delete_files=True, torrent_hashes=self.hash)
            log.debug(f"Deleted from client: {self.name}")
        else:
            self.delete_path(self.content_path)
            self.qbitclient.torrents_add_tags(tags="Deleted", torrent_hashes=self.hash)
...
class Cleaner:
    def __init__(self, config, qbitclient):
        self.config = config
        self.qbitclient = qbitclient
        self.completed_list = self.qbitclient.torrents_info(status_filter="completed")
        self.seeding_list = self.qbitclient.torrents_info(status_filter="seeding")
        self.done_seeding_list = [
            i
            for i in self.completed_list
            if i not in self.seeding_list
            and "Copied" in i.tags
            and "Deleted" not in i.tags
        ]
    ...
    def clean_seeds(self, ignore_age=120):
        if not self.done_seeding_list:
            ...
            for i in [
                ...
                for i in self.done_seeding_list
            ]:
                if (
                    i.time_complete < ignore_age
                    or i.genre not in list(self.config["genres"])
                    or not self.config["genres"][i.genre]["moveToDir"]
                ):
                    continue
                i.delete_content()
                ...
                log.debug(
                    f"Deleted empty directories recursively upstream: {i.save_path}"
                )
