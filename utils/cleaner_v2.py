...
class CompletedSeed:
    ...
    @staticmethod
    def move_files_up_to_root_dir_recursively(dir_path):
        """moves all files in directory tree to root directory
        Args:
            dir_path: top directory in tree
        Returns:
            None
        ...
        for root, dirs, files in os.walk(dir_path, topdown=False):
            [
                (
                    shutil.move(os.path.join(root, file), dir_path),
    ...
    @staticmethod
    def move_all_files_in_tree_to_one_dir(source_dir, dest_dir):
            ...
            for i in [
                ...
                if not i.keep_dir_structure:
                    if os.path.isdir(i.content_path):
                        i.move_all_files_in_tree_to_one_dir(i.content_path, i.save_path)
