...
class CompletedDownload():
    ...
    def __init__(self, name, hash, content_path, save_path, category):
        self.name = name
        self.hash = hash
        ...
        self.destination_dir = os.path.join(config['genres'][self.genre]['moveToDir'], self.category)
        self.keep_dir_structure = config['genres'][self.genre]['keepDirStructure']
        self.file_exts_to_keep = ('*') if not config['genres'][self.genre]['keepSpecificFileTypes'] else tuple(config['genres'][self.genre]['keepSpecificFileTypes'])
    ...
    def list_files_with_exts(self):
        ...
        for ext in self.file_exts_to_keep:
            files.extend(glob.iglob(os.path.join(glob.escape(self.content_path), '**', ext), recursive=True))
        return self.files_to_copy.extend(files)
    ...
    def link_or_copy_tree(self, source, destination_dir):
        ...
        try:
            shutil.copytree(source, destination_path, ignore=self.include_patterns(self.file_exts_to_keep), copy_function=os.link)
            log.debug(f'Hard linked {dir_name} to {destination_dir}')
        except:
            shutil.copytree(source, destination_path, ignore=self.include_patterns(self.file_exts_to_keep))
    ...
    def copy_subtree(self):
        ...
        if str(os.path.dirname(self.content_path)) == str(self.save_path): # then the download is a single file without a directory
            os.makedirs(self.destination_dir, exist_ok=True)
            return self.copy_files_only()
        else:
            parent_dir = self.get_one_fsobj_below(self.content_path, self.save_path) # solves edge case for single file downloads in mutliple layers of directories
    ...
    def check_copy_completed(self):
        ...
        log.debug(f'copied_paths_count: {copied_paths_count}. files_to_copy_count: {files_to_copy_count}. Difference: {copied_paths_count - files_to_copy_count}')
...
class Copier:
    ...
    def identify_completes_to_copy(self):
        completed_torrents = [i for i in client.torrents_info(status_filter='completed') if 'Copied' not in i.tags]
        completed_objs = [CompletedDownload(i.name, i.hash, i.content_path, i.save_path, i.category) for i in completed_torrents]
        return [i for i in completed_objs if i.genre in list(config['genres']) and config['genres'][i.genre]['moveToDir']]
    ...
    def copy_completes(self):
        ...
        for i in self.to_copy:
            ...
            if not i.files_to_copy:
                log.info(f'Completed download had no files with correct extension(s) to copy: {i.name}')
                continue
            ...
            if i.check_copy_completed():
                client.torrents_add_tags(tags='Copied', torrent_hashes=i.hash)
